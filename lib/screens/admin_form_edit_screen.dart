...
class AdminFormEditScreen extends StatefulWidget {
  final int formId;

  const AdminFormEditScreen({
    super.key,
    required this.formId,
  });

  @override
  State<AdminFormEditScreen> createState() => _AdminFormEditScreenState();
}

class _AdminFormEditScreenState extends State<AdminFormEditScreen>
    with TickerProviderStateMixin {
  final _formKey = GlobalKey<FormState>();
  final _titleController = TextEditingController();
  
  List<EditableFormField> formFields = [];
  ...
  Future<void> _loadFormData() async {
    try {
      ...
      final formDetail = await ApiService.getAdminFormDetail(widget.formId);
      
      setState(() {
        _titleController.text = formDetail.title;
        formFields = formDetail.fields.map((field) => EditableFormField.fromSurveyField(field)).toList();
        questionIdCounter = formFields.isNotEmpty ? 
            formFields.map((f) => f.id).reduce((a, b) => a > b ? a : b) + 1 : 0;
        isLoading = false;
      });
    } catch (e) {
      setState(() {
  ...
  void _addQuestion() {
    ...
    final question = EditableFormField(
      id: newQuestionId,
      ...
      condition: EditableFormCondition(
        dependsOn: dependsOn,
  ...
  void _editQuestion(int index) {
    ...
    setState(() {
      _questionLabelEnController.text = field.labelEn;
      _questionLabelFaController.text = field.labelFa;
      _questionLabelArController.text = field.labelAr;
      _conditionValueEnController.text = field.condition.valueEn;
      _conditionValueFaController.text = field.condition.valueFa;
      _conditionValueArController.text = field.condition.valueAr;
      selectedQuestionType = field.type;
      isRequired = field.required;
      dependsOn = field.condition.dependsOn;
      
      // Populate options if they exist
      options.clear();
      for (int i = 0; i < field.optionsEn.length; i++) {
        final optionSet = OptionSet();
        optionSet.en = field.optionsEn.length > i ? field.optionsEn[i] : '';
        optionSet.fa = field.optionsFa.length > i ? field.optionsFa[i] : '';
        optionSet.ar = field.optionsAr.length > i ? field.optionsAr[i] : '';
        optionSet.enController.text = optionSet.en;
        optionSet.faController.text = optionSet.fa;
        optionSet.arController.text = optionSet.ar;
        options.add(optionSet);
      }
    });
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text('Editing: ${field.labelEn}'),
  ...
  Future<void> _updateForm() async {
    ...
    try {
      ...
      final result = await ApiService.updateForm(adminFormData);
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Failed to update form: $e'),
            backgroundColor: Colors.red,
          ),
        );
      }
      ...
      bottomNavigationBar: Container(
        ...
        child: SafeArea(
          child: ElevatedButton(
            onPressed: isSaving ? null : _updateForm,
                    ...
                    child: Column(
                      children: [
                        ...
                        Row(
                          ...
                          children: [
                            const Text('Form ID:', style: TextStyle(fontWeight: FontWeight.bold)),
                            Text('#${widget.formId}'),
                          ],
                        ),
                        ...
                        Row(
                          ...
                          children: [
                            ...
                            Text('${formFields.length} questions'),
                          ],
                        ),
                      ],
                    ),
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    ...formFields.asMap().entries.map((entry) {
                      int index = entry.key;
                      EditableFormField field = entry.value;
                      return Card(
                              ...
                              child: Column(
                                ...
                                children: [
                                  if (field.labelFa.isNotEmpty)
                                    Text('Kurdish: ${field.labelFa}'),
                                  if (field.labelAr.isNotEmpty)
                                    Text('Arabic: ${field.labelAr}'),
                                  ...
                                  if (field.condition.dependsOn.isNotEmpty) ...[
                                    ...
                                    Container(
                                      ...
                                      child: Column(
                                        crossAxisAlignment: CrossAxisAlignment.start,
                                        children: [
                                          const Text('Conditional Logic:', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 12)),
                                          const SizedBox(height: 4),
                                          Text('Shows when question ${field.condition.dependsOn} equals:', style: TextStyle(fontSize: 11)),
                                          const SizedBox(height: 4),
                                          if (field.condition.valueEn.isNotEmpty)
                                            Text('• English: "${field.condition.valueEn}"', style: TextStyle(fontSize: 10)),
                                          if (field.condition.valueFa.isNotEmpty)
                                            Text('• Kurdish: "${field.condition.valueFa}"', style: TextStyle(fontSize: 10)),
                                          if (field.condition.valueAr.isNotEmpty)
                                            Text('• Arabic: "${field.condition.valueAr}"', style: TextStyle(fontSize: 10)),
                                        ],
                                      ),
                                    ),
                                  ],
        ...
        child: Column(
          ...
          children: [
            ...
            if (dependsOn.isNotEmpty) ...[
              ...
              Builder(
                builder: (context) {
                  EditableFormField? selectedField;
                  try {
                    selectedField = formFields.cast<EditableFormField?>().firstWhere(
...
class EditableFormField {
  ...
  final EditableFormCondition condition;
  final bool required;
  ...
  factory EditableFormField.fromSurveyField(SurveyField surveyField) {
    return EditableFormField(
      id: surveyField.id,
      type: surveyField.type,
      labelEn: surveyField.labelEn,
      labelFa: surveyField.labelFa,
      labelAr: surveyField.labelAr,
      optionsEn: surveyField.optionsEn,
      optionsFa: surveyField.optionsFa,
      optionsAr: surveyField.optionsAr,
      condition: EditableFormCondition(
        dependsOn: surveyField.condition.dependsOn,
        valueEn: surveyField.condition.valueEn,
        valueFa: surveyField.condition.valueFa,
        valueAr: surveyField.condition.valueAr,
      ),
...
class EditableFormCondition {
  final String dependsOn;
  final String valueEn;
  final String valueFa;
  final String valueAr;

  EditableFormCondition({
